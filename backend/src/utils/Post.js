...
const fileHandle = (module.exports = {
  ...
  rootFolderHandle: (type) => {
    ...
    if (type === "post-pic" || type === "post-file")
      rootFolder = process.env.Folder_Post_ID;
    if (type === "user") rootFolder = process.env.Folder_User_ID;
    if (type === "chat-pic" || type === "chat-file")
      rootFolder = process.env.Folder_Chat_ID;
    return rootFolder;
  },
    ...
    try {
      ...
      const createFile = await drive.files.create({
        requestBody: {
          name: fileName,
          mimeType: "image/jpg",
          parents: ["1R9qK5OlU4rd_9TOrJDYapivrNI4PS4mz"],
        },
        media: {
          mimeType: "image/jpg",
      ...
      const display_url = `https://drive.google.com/thumbnail?id=${createFile.data.id}&sz=w700`;
      const data = {
